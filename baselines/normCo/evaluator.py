...
class Evaluator():
    ...
    def accu(self,score_matrix,top_k):
        """
        inputs:
        ...
        """
        scores,preds = get_sorted_top_k(score_matrix,top_k=top_k,reverse=False) #preds: shape(n_samples,top_k)
        # temp = (preds.reshape(-1,1).repeat(top_k,axis=-1))
        correctness = preds==0
        l = preds.shape[0]
        correctness = correctness.astype("float")
        return correctness.sum()/l,correctness.sum(),l
